...
function sortingLoop() {
    ...
    let program = document.getElementById('studentProgram').value;
    let semester = Number(document.getElementById('studentSemester').value);

    resultElement.innerHTML = '';
    for(let currentStudentIndex = 0; currentStudentIndex < allStudents.length; currentStudentIndex++)
    {
        //Use this condition and the filters to only display select students
        if(((program  === allStudents[currentStudentIndex].program        ) || program  === "All") &&
           ((semester === allStudents[currentStudentIndex].currentSemester) || semester === 0    ))
        {
            let currentStudentElement = allStudents[currentStudentIndex].createElement();
